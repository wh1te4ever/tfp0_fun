...
uint64_t pin_current_thread() {
  ...
  kwrite64(th + koffset(KSTRUCT_OFFSET_THREAD_BOUND_PROCESSOR), processor);
#endif
  ...
  uint64_t cpu_data = kread64(cpu_data_entries + ((cpu_id * 0x10) + 8));
  ...
  for (int i = 0; i < 1000; i++) {
    swtch_pri(0);
    uint64_t p = kread64(th + koffset(KSTRUCT_OFFSET_THREAD_CHOSEN_PROCESSOR));
    if (p != processor) {
    ...
    usleep(15000);
    p = kread64(th + koffset(KSTRUCT_OFFSET_THREAD_CHOSEN_PROCESSOR));
    if (p != processor) {
      printf("got moved off target processor\n");
      got_switched = 1;
      break;
...
void do_syscall_with_pstate_d_unmasked(struct syscall_args* args) {
  ...
  //uint64_t thread_t_addr = kread64(thread_port_addr + koffset(KSTRUCT_OFFSET_IPC_PORT_IP_KOBJECT));
  ...
  uint64_t act_context = kread64(thread_t_addr + off_thread_context_data);
  eret_return_state.ss.ss_64.x[21] = act_context;
  
  // let's stay on the thread's actual kernel stack
  uint64_t thread_kernel_stack_top = kread64(thread_t_addr + off_thread_kstackptr);
...
void handle_kernel_bp_hits(mach_port_t target_thread_port, uint64_t infinite_looper_pc, breakpoint_callback callback) {
  ...
  while (1) {
    ...
    while (1) {
      ...
      uint64_t kstackptr = kread64(thread_t_addr + off_thread_kstackptr);
